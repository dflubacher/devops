...
NAME='debian-bullseye-preseed'
DESCRIPTION='Debian 11 preseeded'
VCPUS=2
MEMORY=2048
...
ISO_FILE=${2:-"/tmp/debian-amd64-preseed.iso"}
...
main() {
    ...
    if ${MACVTAP}; then
        virt-install --name "${NAME}" --description "${DESCRIPTION}" \
                    ...
                    --cdrom "${ISO_FILE}" \
                    --hvm --os-variant=debian10 --noautoconsole
    else
        virt-install --name "${NAME}" --description "${DESCRIPTION}" \
                    --memory ${MEMORY} \
                    --vcpus=2 --cpu host \
